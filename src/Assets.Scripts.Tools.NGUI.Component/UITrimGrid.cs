...
{
	public class UITrimGrid : UIGrid
	{
		public Func<Transform, int, Vector3, Vector3> ResetPositionHook;
		...
		protected override void ResetPosition(List<Transform> list)
		{
			...
			while (i < count)
			{
				...
				if (this.ResetPositionHook != null)
				{
					vector = this.ResetPositionHook(transform2, i, vector);
				}
			...
			if (this.pivot != UIWidget.Pivot.TopLeft)
			{
				...
				for (int j = 0; j < transform.childCount; j++)
				{
					...
					if (component != null)
					{
						SpringPosition expr_28A_cp_0 = component;
						expr_28A_cp_0.target.x = expr_28A_cp_0.target.x - num5;
						SpringPosition expr_29F_cp_0 = component;
						expr_29F_cp_0.target.y = expr_29F_cp_0.target.y - num6;
					}
					else
					{
