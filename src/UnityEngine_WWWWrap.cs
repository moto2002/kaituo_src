...
public class UnityEngine_WWWWrap
{
	...
	private static int GetAudioClip(IntPtr L)
	{
		...
		try
		{
			...
			else if (num == 4 && TypeChecker.CheckTypes(L, 1, typeof(WWW), typeof(bool), typeof(bool), typeof(uint)))
			{
				WWW wWW3 = (WWW)ToLua.ToObject(L, 1);
				bool threeD3 = LuaDLL.lua_toboolean(L, 2);
				bool stream2 = LuaDLL.lua_toboolean(L, 3);
				AudioType audioType = (AudioType)LuaDLL.lua_tonumber(L, 4);
	...
	private static int GetAudioClipCompressed(IntPtr L)
	{
		...
		try
		{
			...
			else if (num == 3 && TypeChecker.CheckTypes(L, 1, typeof(WWW), typeof(bool), typeof(uint)))
			{
				WWW wWW3 = (WWW)ToLua.ToObject(L, 1);
				bool threeD2 = LuaDLL.lua_toboolean(L, 2);
				AudioType audioType = (AudioType)LuaDLL.lua_tonumber(L, 3);
	...
	private static int set_threadPriority(IntPtr L)
	{
		...
		try
		{
			obj = ToLua.ToObject(L, 1);
			WWW wWW = (WWW)obj;
			ThreadPriority threadPriority = (ThreadPriority)LuaDLL.luaL_checknumber(L, 2);
