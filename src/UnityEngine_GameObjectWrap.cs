...
public class UnityEngine_GameObjectWrap
{
	...
	private static int CreatePrimitive(IntPtr L)
	{
		...
		try
		{
			...
			PrimitiveType type = (PrimitiveType)LuaDLL.luaL_checknumber(L, 1);
	...
	private static int SendMessageUpwards(IntPtr L)
	{
		...
		try
		{
			...
			else if (num == 3 && TypeChecker.CheckTypes(L, 1, typeof(GameObject), typeof(string), typeof(uint)))
			{
				GameObject gameObject2 = (GameObject)ToLua.ToObject(L, 1);
				string methodName2 = ToLua.ToString(L, 2);
				SendMessageOptions options = (SendMessageOptions)LuaDLL.lua_tonumber(L, 3);
			...
			else if (num == 4 && TypeChecker.CheckTypes(L, 1, typeof(GameObject), typeof(string), typeof(object), typeof(uint)))
			{
				...
				SendMessageOptions options2 = (SendMessageOptions)LuaDLL.lua_tonumber(L, 4);
	...
	private static int get_scene(IntPtr L)
	{
		...
		{
			result = LuaDLL.toluaL_exception(L, ex, (obj != null) ? ex.Message : "attempt to index scene on a nil value");
		}
		return result;
	}
