...
namespace LuaInterface
{
	public class LuaTable : LuaBaseRef
	{
		...
		public string GetStringField(string _name)
		{
			int newTop = this.luaState.LuaGetTop();
			string result;
			try
			{
				this.luaState.Push(this);
				this.luaState.LuaGetField(-1, _name);
		...
		public override string ToString()
		{
			...
			this.luaState.LuaPop(1);
			return string.Format("table:0x{0}({1})", intPtr.ToString("X"), this.name);
		}
