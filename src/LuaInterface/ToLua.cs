	...
	public static class ToLua
	{
		...
		private static int Loader(IntPtr L)
		{
			...
			try
			{
				...
				byte[] array;
				int size;
				LuaFileUtils.Instance.ReadFile(text, out array, out size);
				if (array == null)
				{
				...
				else
				{
					...
					if (LuaDLL.luaL_loadbuffer(L, array, size, text) != 0)
					{
		...
		public static int DoFile(IntPtr L)
		{
			...
			try
			{
				...
				if (LuaDLL.luaL_loadbuffer(L, array, size, text) == 0 && LuaDLL.lua_pcall(L, 0, LuaDLL.LUA_MULTRET, 0) != 0)
				{
					string msg = LuaDLL.lua_tostring(L, -1);
					throw new LuaException(msg, LuaException.GetLastError(), 1);
				}
		...
		private static void PushUserData(IntPtr L, object o, int reference)
		{
			ObjectTranslator objectTranslator = ObjectTranslator.Get(L);
			int num;
			if (objectTranslator.Getudata(o, out num))
			{
				if (LuaDLL.tolua_pushudata(L, num))
				{
					return;
				}
				objectTranslator.Destroyudata(num);
			}
			num = objectTranslator.AddObject(o);
			LuaDLL.tolua_pushnewudata(L, reference, num);
		}
