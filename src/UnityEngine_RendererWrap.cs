...
public class UnityEngine_RendererWrap
{
	...
	private static int set_shadowCastingMode(IntPtr L)
	{
		...
		try
		{
			obj = ToLua.ToObject(L, 1);
			Renderer renderer = (Renderer)obj;
			ShadowCastingMode shadowCastingMode = (ShadowCastingMode)LuaDLL.luaL_checknumber(L, 2);
	...
	private static int set_reflectionProbeUsage(IntPtr L)
	{
		...
		try
		{
			obj = ToLua.ToObject(L, 1);
			Renderer renderer = (Renderer)obj;
			ReflectionProbeUsage reflectionProbeUsage = (ReflectionProbeUsage)LuaDLL.luaL_checknumber(L, 2);
