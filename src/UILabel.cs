...
public class UILabel : UIWidget
{
	...
	[NonSerialized]
	protected string mProcessedText;

	[NonSerialized]
	private bool mPremultiply;

	[NonSerialized]
	protected Vector2 mCalculatedSize = Vector2.zero;

	[NonSerialized]
	protected float mScale = 1f;

	[NonSerialized]
	protected int mFinalFontSize;
	...
	protected bool shouldBeProcessed
	{
		get
		{
	...
	protected Vector2 labelMEffectDistance
	{
		get
		{
			return this.mEffectDistance;
	...
	private static void OnFontChanged(Font font)
	{
		for (int i = 0; i < UILabel.mList.size; i++)
		{
			...
			if (uILabel != null)
			{
				...
				{
					trueTypeFont.RequestCharactersInTexture(uILabel.printedText, uILabel.mFinalFontSize, uILabel.mFontStyle);
	...
	public virtual void ProcessText()
	{
	...
	private void ProcessText(bool legacyMode, bool full)
	{
		...
		if (this.mFinalFontSize > 0)
		{
			...
			for (int i = this.mFinalFontSize; i > 0; i--)
			{
				...
				bool flag2 = NGUIText.WrapText(this.printedText, out this.mProcessedText, true, false, this.mOverflowEllipsis && this.mOverflow == UILabel.Overflow.ClampContent);
	...
	public void ApplyShadow(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols, int start, int end, float x, float y)
	{
		...
		for (int i = start; i < end; i++)
		{
			...
			else
			{
				...
				cols.buffer[i] = c.GammaToLinearSpace();
			}
		}
	}
