...
public class UnityEngine_InputWrap
{
	...
	private static int GetKey(IntPtr L)
	{
		...
		try
		{
			...
			if (num == 1 && TypeChecker.CheckTypes(L, 1, typeof(uint)))
			{
				KeyCode key = (KeyCode)LuaDLL.lua_tonumber(L, 1);
	...
	private static int set_imeCompositionMode(IntPtr L)
	{
		...
		try
		{
			IMECompositionMode imeCompositionMode = (IMECompositionMode)LuaDLL.luaL_checknumber(L, 2);
