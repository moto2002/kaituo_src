...
public class UIInput : MonoBehaviour
{
	...
	public void AddOnChange(EventDelegate.Callback del)
	{
		EventDelegate.Add(this.onChange, del);
	}

	public void AddOnChange(EventDelegate del)
	{
		EventDelegate.Add(this.onChange, del);
	}

	public void RemoveOnChange(EventDelegate del)
	{
		if (this.onChange != null)
		{
			this.onChange.Remove(del);
		}
	}
	...
	protected virtual void Insert(string text)
	{
		...
		int num = 0;
		if (this.utf8LimitCheck)
		{
			int num2 = Encoding.UTF8.GetByteCount(this.mValue) - this.mValue.Length;
			num = this.mValue.Length - (num2 >> 1) + num2;
		}
		...
		while (i < length2)
		{
			...
			else
			{
				...
				{
					num += ((c <= '\u007f') ? 1 : 2);
					if (this.characterLimit > 0 && num > this.characterLimit)
					{
						break;
					}
				}
	...
	protected char Validate(string text, int pos, char ch)
	{
		...
		case UIInput.Validation.Float:
			...
			break;
		case UIInput.Validation.Alphanumeric:
			if (ch >= 'A' && ch <= 'Z')
			{
				return ch;
			}
			...
			break;
		case UIInput.Validation.Username:
			if (ch >= 'A' && ch <= 'Z')
			{
				return ch - 'A' + 'a';
			}
		...
		case UIInput.Validation.Filename:
			if (ch == ':')
			{
				return '\0';
			}
			...
			return ch;
		case UIInput.Validation.NoSymbol:
