...
public class UnityEngine_AsyncOperationWrap
{
	...
	private static int get_priority(IntPtr L)
	{
		...
		try
		{
			obj = ToLua.ToObject(L, 1);
			AsyncOperation asyncOperation = (AsyncOperation)obj;
			int priority = asyncOperation.priority;
	...
	private static int get_allowSceneActivation(IntPtr L)
	{
		...
		try
		{
			obj = ToLua.ToObject(L, 1);
			AsyncOperation asyncOperation = (AsyncOperation)obj;
			bool allowSceneActivation = asyncOperation.allowSceneActivation;
			LuaDLL.lua_pushboolean(L, allowSceneActivation);
			result = 1;
		}
		catch (Exception ex)
		{
			result = LuaDLL.toluaL_exception(L, ex, (obj != null) ? ex.Message : "attempt to index allowSceneActivation on a nil value");
		}
		return result;
	}
