...
public class UnityEngine_ApplicationWrap
{
	...
	private static int RequestAdvertisingIdentifierAsync(IntPtr L)
	{
		...
		try
		{
			...
			{
				delegateMethod = (Application.AdvertisingIdentifierCallback)ToLua.CheckObject(L, 1, typeof(Application.AdvertisingIdentifierCallback));
			}
			else
			{
				LuaFunction func = ToLua.ToLuaFunction(L, 1);
				delegateMethod = (DelegateFactory.CreateDelegate(typeof(Application.AdvertisingIdentifierCallback), func) as Application.AdvertisingIdentifierCallback);
			}
			bool value = Application.RequestAdvertisingIdentifierAsync(delegateMethod);
			LuaDLL.lua_pushboolean(L, value);
			result = 1;
		}
		catch (Exception e)
	...
	private static int RequestUserAuthorization(IntPtr L)
	{
		...
		{
			ToLua.CheckArgsCount(L, 1);
			UserAuthorization mode = (UserAuthorization)LuaDLL.luaL_checknumber(L, 1);
	...
	private static int set_stackTraceLogType(IntPtr L)
	{
		...
		try
		{
			StackTraceLogType stackTraceLogType = (StackTraceLogType)LuaDLL.luaL_checknumber(L, 2);
	...
	private static int set_backgroundLoadingPriority(IntPtr L)
	{
		...
		try
		{
			ThreadPriority backgroundLoadingPriority = (ThreadPriority)LuaDLL.luaL_checknumber(L, 2);
