...
{
	public class SocketChannelL : ISocketChannel
	{
		...
		public SocketChannelL(string id)
		{
			...
			this._recvStream = new SprotoStreamEx(SocketChannelL.RECV_BUFFER_SIZE);
			this._sendStream = new SprotoStreamEx(SocketChannelL.SEND_BUFFER_SIZE);
		...
		public bool Send(LuaByteBuffer message)
		{
			...
			if (this.IsConnected() && message != null && message.buffer.Length > 0)
			{
				...
				lock (sendStream)
				{
					...
					if (num > SocketChannelL.MAX_PACK_LEN)
					{
		...
		public void Connect(string host, int port)
		{
			try
			{
				...
				Debug.Log(string.Format("SocketDriver connecting to {0}:{1}", host, port));
			}
			catch (Exception ex)
			{
		...
		private void OnReceive(IAsyncResult ar)
		{
			...
			if (ar != null)
			{
				try
				{
					...
					bool flag = this._recvStream.AddMessageSize(size);
				}
				catch (Exception ex)
				{
