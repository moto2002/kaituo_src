...
public class UnityEngine_ComponentWrap
{
	...
	private static int SendMessageUpwards(IntPtr L)
	{
		...
		try
		{
			...
			else if (num == 3 && TypeChecker.CheckTypes(L, 1, typeof(Component), typeof(string), typeof(uint)))
			{
				Component component2 = (Component)ToLua.ToObject(L, 1);
			...
			else if (num == 4 && TypeChecker.CheckTypes(L, 1, typeof(Component), typeof(string), typeof(object), typeof(uint)))
			{
