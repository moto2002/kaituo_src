...
{
	public static class NGUIMathLua
	{
		...
		{
			return NGUIMathLua.IntToColor((int)val);
		}
		...
		{
			rect = NGUIMathLua.ConvertToPixels(rect, width, height, true);
			rect.xMin = (float)Mathf.RoundToInt(rect.xMin);
			rect.yMin = (float)Mathf.RoundToInt(rect.yMin);
			rect.xMax = (float)Mathf.RoundToInt(rect.xMax);
			rect.yMax = (float)Mathf.RoundToInt(rect.yMax);
			return NGUIMathLua.ConvertToTexCoords(rect, width, height);
		}
		...
		{
			return NGUIMathLua.CalculateRelativeWidgetBounds(trans, trans, false, true);
		}

		public static Bounds CalculateRelativeWidgetBounds(Transform trans, bool considerInactive)
		{
			return NGUIMathLua.CalculateRelativeWidgetBounds(trans, trans, considerInactive, true);
		}

		public static Bounds CalculateRelativeWidgetBounds(Transform relativeTo, Transform content)
		{
			return NGUIMathLua.CalculateRelativeWidgetBounds(relativeTo, content, false, true);
		}
		...
		public static Bounds CalculateRelativeWidgetBounds(Transform relativeTo, Transform content, bool considerInactive, bool considerChildren = true)
		{
			if (content != null && relativeTo != null)
			{
				...
				Vector3 center = new Vector3(3.40282347E+38f, 3.40282347E+38f, 3.40282347E+38f);
				Vector3 point = new Vector3(-3.40282347E+38f, -3.40282347E+38f, -3.40282347E+38f);
				NGUIMathLua.CalculateRelativeWidgetBounds(content, considerInactive, true, ref worldToLocalMatrix, ref center, ref point, ref flag, considerChildren);
				if (flag)
				{
					Bounds result = new Bounds(center, Vector3.zero);
					result.Encapsulate(point);
		...
		private static void CalculateRelativeWidgetBounds(Transform content, bool considerInactive, bool isRoot, ref Matrix4x4 toLocal, ref Vector3 vMin, ref Vector3 vMax, ref bool isSet, bool considerChildren)
		{
			...
			else
			{
				...
				while (k < childCount)
				{
					NGUIMathLua.CalculateRelativeWidgetBounds(content.GetChild(k), considerInactive, false, ref toLocal, ref vMin, ref vMax, ref isSet, true);
					k++;
				}
			}
		}
		...
		{
			return Vector2.Lerp(from, to, NGUIMathLua.SpringLerp(strength, deltaTime));
		}

		public static Vector3 SpringLerp(Vector3 from, Vector3 to, float strength, float deltaTime)
		{
			return Vector3.Lerp(from, to, NGUIMathLua.SpringLerp(strength, deltaTime));
		}

		public static Quaternion SpringLerp(Quaternion from, Quaternion to, float strength, float deltaTime)
		{
			return Quaternion.Slerp(from, to, NGUIMathLua.SpringLerp(strength, deltaTime));
		}

		public static float RotateTowards(float from, float to, float maxAngle)
		{
			float num = NGUIMathLua.WrapAngle(to - from);
		...
		public static float DistanceToRectangle(Vector2[] screenPoints, Vector2 mousePos)
		{
			...
			if (!flag)
			{
				...
				for (int j = 0; j < 4; j++)
				{
					...
					float num2 = NGUIMathLua.DistancePointToLineSegment(mousePos, v, v2);
					if (num2 < num || num < 0f)
					{
						num = num2;
					}
		...
		public static float DistanceToRectangle(Vector3[] worldPoints, Vector2 mousePos, Camera cam)
		{
			...
			return NGUIMathLua.DistanceToRectangle(array, mousePos);
		}
		...
		{
			NGUIMathLua.ResizeWidget(w, pivot, x, y, 2, 2, 100000, 100000);
		}
		...
		public static void ResizeWidget(UIWidget w, UIWidget.Pivot pivot, float x, float y, int minWidth, int minHeight, int maxWidth, int maxHeight)
		{
			if (pivot == UIWidget.Pivot.Center)
			{
				...
				if ((num | num2) != 0)
				{
					num >>= 1;
					num2 >>= 1;
					NGUIMathLua.AdjustWidget(w, (float)(-(float)num), (float)(-(float)num2), (float)num, (float)num2, minWidth, minHeight);
				}
				return;
			}
			...
			case UIWidget.Pivot.TopLeft:
				NGUIMathLua.AdjustWidget(w, point.x, 0f, 0f, point.y, minWidth, minHeight, maxWidth, maxHeight);
				break;
			case UIWidget.Pivot.Top:
				NGUIMathLua.AdjustWidget(w, 0f, 0f, 0f, point.y, minWidth, minHeight, maxWidth, maxHeight);
				break;
			case UIWidget.Pivot.TopRight:
				NGUIMathLua.AdjustWidget(w, 0f, 0f, point.x, point.y, minWidth, minHeight, maxWidth, maxHeight);
				break;
			case UIWidget.Pivot.Left:
				NGUIMathLua.AdjustWidget(w, point.x, 0f, 0f, 0f, minWidth, minHeight, maxWidth, maxHeight);
				break;
			case UIWidget.Pivot.Right:
				NGUIMathLua.AdjustWidget(w, 0f, 0f, point.x, 0f, minWidth, minHeight, maxWidth, maxHeight);
				break;
			case UIWidget.Pivot.BottomLeft:
				NGUIMathLua.AdjustWidget(w, point.x, point.y, 0f, 0f, minWidth, minHeight, maxWidth, maxHeight);
				break;
			case UIWidget.Pivot.Bottom:
				NGUIMathLua.AdjustWidget(w, 0f, point.y, 0f, 0f, minWidth, minHeight, maxWidth, maxHeight);
				break;
			case UIWidget.Pivot.BottomRight:
				NGUIMathLua.AdjustWidget(w, 0f, point.y, point.x, 0f, minWidth, minHeight, maxWidth, maxHeight);
				break;
			}
		}
		...
		{
			NGUIMathLua.AdjustWidget(w, left, bottom, right, top, 2, 2, 100000, 100000);
		}

		public static void AdjustWidget(UIWidget w, float left, float bottom, float right, float top, int minWidth, int minHeight)
		{
			NGUIMathLua.AdjustWidget(w, left, bottom, right, top, minWidth, minHeight, 100000, 100000);
		}
